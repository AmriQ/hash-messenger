...
function register_user($nama, $user, $sandi)
{
  ...
  $query = "INSERT INTO users (name, username, password) VALUES ('$nama', '$user', '$sandi')";
...
function nama_pengirim($nama)
{
  ...
  $query = "SELECT name FROM users WHERE username='$nama'";
  if ($hasil = mysqli_query($link, $query)) {
...
function reset_password($user, $sandi)
{
  ...
  $user  = mysqli_real_escape_string($link, $user);
  $sandi = mysqli_real_escape_string($link, $sandi);
  $query = "UPDATE users SET password='$sandi' WHERE username='$user'";
  if (mysqli_query($link, $query)) {
    return true;
  } else {
    return false;
